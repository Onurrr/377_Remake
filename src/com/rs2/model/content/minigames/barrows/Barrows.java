			...
			case 10284: // chest
				if (player.getKillCount() < 1) {
					player.getActionSender().sendMessage(
							"You must kill the the brother before searching this.");
					return true;
				}
				for (int x = 0; x < barrowsBrothers.length; x++) {
					if (x == player.getRandomGrave()) {
						if (player.getKillCount() > 5) {
							getReward(player);
							return true;
						} else if (NpcLoader.checkSpawn(player, barrowsBrothers[x][1])) {
						}
						if (!player.getBarrowsNpcDead()[x]) {
							NpcLoader.spawnNpc(player, new Npc(
									barrowsBrothers[x][1]), true, true);
							return true;
						}
						break;
					}
